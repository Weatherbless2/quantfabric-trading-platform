...
struct XServerConfig
{
    ...
    int AuthorizationTimeoutMs = 1000;
    string AuthorizationDomain = "desk:cn_equity";
    bool BusinessPolicyEnabled = false;
    string BusinessPolicyURL = "http://127.0.0.1:19080";
    int BusinessPolicyTimeoutMs = 1000;
    int BusinessPolicyRefreshSeconds = 60;
};
...
static bool LoadXServerConfig(const char *yml, XServerConfig& ret, string& out)
{
    ...
    try
    {
        ...
        YAML::Node authorizationConfig = sourceConfig["Authorization"];
        if(authorizationConfig)
        {
            ret.AuthorizationEnabled = authorizationConfig["Enabled"].as<bool>();
            ret.AuthorizationURL = authorizationConfig["ServiceURL"].as<string>();
            ret.AuthorizationTimeoutMs = authorizationConfig["TimeoutMs"].as<int>();
            ret.AuthorizationDomain = authorizationConfig["Domain"].as<string>();
        }
        YAML::Node businessPolicyConfig = sourceConfig["BusinessPolicy"];
        if(businessPolicyConfig)
        {
            ret.BusinessPolicyEnabled = businessPolicyConfig["Enabled"].as<bool>();
            ret.BusinessPolicyURL = businessPolicyConfig["ServiceURL"].as<string>();
            ret.BusinessPolicyTimeoutMs = businessPolicyConfig["TimeoutMs"].as<int>();
            ret.BusinessPolicyRefreshSeconds = businessPolicyConfig["RefreshSeconds"].as<int>();
            if(ret.BusinessPolicyTimeoutMs <= 0 || ret.BusinessPolicyRefreshSeconds <= 0)
            {
                out = "BusinessPolicy timeout and refresh seconds must be positive";
                return false;
            }
        }
    }
...
struct XMonitorConfig
{
    string XServerIP;
    int XServerPort;
    string AuthorizationServiceURL = "http://127.0.0.1:18080";
    string UserName;
    string PassWord;
...
static bool LoadXMonitorConfig(const char *yml, XMonitorConfig& ret, string& out)
{
    ...
    try
    {
        ...
        ret.XServerIP = sourceConfig["XServerIP"].as<string>();
        ret.XServerPort = sourceConfig["XServerPort"].as<int>();
        if(sourceConfig["AuthorizationServiceURL"])
        {
            ret.AuthorizationServiceURL = sourceConfig["AuthorizationServiceURL"].as<string>();
        }
