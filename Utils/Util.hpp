...
static const char *getCurrentTimeSec()
{
    ...
    static thread_local char szBuffer[64] = {0};
...
static const char *getCurrentTimeMs()
{
    ...
    localtime_r(&current, &timeStamp);
    static thread_local char szBuffer[64] = {0};
    size_t length = strftime(szBuffer, sizeof(szBuffer), "%Y-%m-%d %H:%M:%S", &timeStamp);
    unsigned long mod = 1e3;
    unsigned long ret = n % mod;
    snprintf(szBuffer + length, sizeof(szBuffer) - length, ".%03lu", ret);
    return szBuffer;
}
...
static const char *getCurrentTimeUs()
{
    ...
    unsigned long ret = n % mod;
    snprintf(szBuffer + length, sizeof(szBuffer) - length, ".%06lu", ret);
    return szBuffer;
}
...
static const char *getCurrentTimeNs()
{
    ...
    unsigned long ret = n % mod;
    snprintf(szBuffer + length, sizeof(szBuffer) - length, ".%09lu", ret);
    return szBuffer;
}
