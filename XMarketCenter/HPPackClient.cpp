...
Utils::LockFreeQueue<Message::PackMessage> HPPackClient::m_MarketMessageQueue;
...
void HPPackClient::Login(const char* Account)
{
    Message::PackMessage message;
    message.MessageType = Message::EMessageType::ELoginRequest;
    message.LoginRequest.ClientType = Message::EClientType::EXMARKETCENTER;
    strncpy(message.LoginRequest.Account, Account, sizeof(message.LoginRequest.Account));
    SendData((const unsigned char*)&message, sizeof (message));
    memcpy(&m_LoginRequest, &message.LoginRequest, sizeof(m_LoginRequest));
}

void HPPackClient::ReConnect()
{
    if (!m_Connected)
    {
        Start();
...
En_HP_HandleResult __stdcall HPPackClient::OnReceive(HP_Server pSender, HP_CONNID dwConnID, const BYTE *pData, int iLength)
{
    ...
    {
        Utils::gLogger->Log->warn(
            "HPPackClient::OnReceive rejected invalid PackMessage size:{} expected:{}",
...
En_HP_HandleResult __stdcall HPPackClient::OnClose(HP_Server pSender, HP_CONNID dwConnID, En_HP_SocketOperation enOperation, int iErrorCode)
{
    ...
    char errorString[512] = {0};
    sprintf(errorString, "HPPackClient::OnClose connID:%llu %s:%d closed, sys error code:%d, error message:%s",
            static_cast<unsigned long long>(dwConnID), szAddress, usPort,
            SYS_GetLastError(), ::HP_Client_GetLastErrorDesc(pSender));
    Utils::gLogger->Log->warn(errorString);
    return HR_OK;
}
