...
{
    // 业务链路起点：柜台插件产生行情；本进程同时把行情发布到共享内存，
    // 并通过 XWatcher -> XServer 转发给 XMonitor。
    // 这个 HPPackClient 是监控链路客户端，不参与策略读取行情的低延迟路径。
    ...
    // Update App Status
    InitAppStatus();
    // 创建共享内存发布端。XQuant 以 MarketServerName 连接它并读取行情。
    ...
    // Pull 线程：调用柜台/测试插件，把原始行情写入插件内部无锁队列。
    m_pPullThread = new std::thread(&XMarketCenter::PullMarketData, this);
    // Handle 线程：校验并扇出到共享内存策略链路和网络监控链路。
...
void XMarketCenter::PullMarketData()
{
    ...
    if(NULL != m_MarketGateWay)
    {
        ...
        // 插件的回调不直接调用策略，避免柜台 API 线程被后续处理阻塞。
        m_MarketGateWay->Run();
    }
}
...
void XMarketCenter::HandleMarketData()
{
    ...
    if(m_MarketCenterConfig.BusinessType == Message::EBusinessType::EFUTURE)
    {
        ...
        while (true)
        {
            // 从插件无锁队列取行情。此处是"柜台 API -> MarketServer"的交接点。
            while (true)
            {
    ...
    else if(m_MarketCenterConfig.BusinessType == Message::EBusinessType::ESTOCK)
    {
        while(true)
        {
            // Control messages use the same C++ route as other monitor
            // commands; the market plugin translates them for its vendor API.
            while(m_PackClient->m_MarketMessageQueue.Pop(msg))
            {
                if(msg.MessageType == Message::EMessageType::ECommand)
                {
                    m_MarketGateWay->HandleCommand(msg.Command);
                }
            }
