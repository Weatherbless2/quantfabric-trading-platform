...
import socket
import sys
# from syslog import LOG_DEBUG, LOG_INFO
import pandas as pd
...
from OxQuant.pytdx.errors import TdxConnectionError, TdxFunctionCallError
...
from OxQuant.pytdx.parser.raw_parser import RawParser
    ...
    def wrapper(self, *args, **kw):
        ...
        try:
            ...
            if self.raise_exception:
                to_raise = TdxFunctionCallError(f"calling function error[{str(e)}]")
...
class BaseSocketClient(object):
    ...
    def connect(self, ip='101.227.73.20', port=7709, time_out=CONNECT_TIMEOUT, bindport=None, bindip='0.0.0.0'):
        ...
        log.info(f"connecting to server : {ip} on port :{port} timeout :{time_out}")
