...
class HPPackClient: public QObject
{
    Q_OBJECT
public:
    HPPackClient(const char *ip, unsigned int port, const char* UserName, const char* SessionID);
    void Start();
    void SetLoginSuccessed(bool ok = true);
    void Stop();
    static void SendData(const unsigned char *pBuffer, int iLength);
    static bool IsLoginSuccessed();
    virtual ~HPPackClient();
private:
    void ConnectServer();
    void Login(const char* UserName, const char* SessionID);
signals:
...
private:
    ...
    static HPPackClient* m_ActiveClient;
    static bool m_IsConnected;
    static std::atomic_bool m_LoginSuccessed;
