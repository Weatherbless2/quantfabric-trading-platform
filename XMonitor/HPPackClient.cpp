...
std::atomic_bool HPPackClient::m_LoginSuccessed(false);
int HPPackClient::m_UUID = 0;

HP_Client HPPackClient::hpClient;
HPPackClient* HPPackClient::m_ActiveClient = nullptr;

HPPackClient::HPPackClient(const char *ip, unsigned int port, const char* UserName, const char* SessionID)
{
    m_Stop = false;
    m_Login = false;
    m_ServerIP = ip;
...
void HPPackClient::Login(const char* UserName, const char* SessionID)
{
    Message::PackMessage message{};
    message.MessageType = Message::EMessageType::ELoginRequest;
    message.LoginRequest.ClientType = Message::EClientType::EXMONITOR;
    Utils::CopyString(message.LoginRequest.Account, UserName);
    // Passwords are only sent to AuthAdminService over its login endpoint.
    // XServer receives the short opaque session through the fixed UUID field.
    Utils::CopyString(message.LoginRequest.UUID, SessionID);
...
{
    m_Login = ok;
    m_LoginSuccessed.store(ok);
}

bool HPPackClient::IsLoginSuccessed()
{
    return m_LoginSuccessed.load();
}
...
En_HP_HandleResult __stdcall HPPackClient::OnConnect(HP_Client pSender, HP_CONNID dwConnID)
{
    ...
    if(m_ActiveClient)
    {
        m_ActiveClient->Login(m_ActiveClient->m_UserName.toStdString().c_str(),
                              m_ActiveClient->m_SessionID.toStdString().c_str());
    }
    return HR_OK;
}
...
En_HP_HandleResult __stdcall HPPackClient::OnReceive(HP_Client pSender, HP_CONNID dwConnID, const BYTE *pData, int iLength)
{
    ...
    {
        FMTLOG(fmtlog::WRN, "HPPackClient::OnReceive rejected invalid PackMessage size:{} expected:{}",
                iLength, sizeof(Message::PackMessage));
        return HR_OK;
    }
    Message::PackMessage message{};
    memcpy(&message, pData, sizeof(message));
    if(message.MessageType == Message::EMessageType::ELoginResponse && message.LoginResponse.ErrorID == 0)
    {
        m_LoginSuccessed.store(true);
    }
