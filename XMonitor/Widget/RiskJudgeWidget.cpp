...
void RiskJudgeWidget::InitFilterWidget()
{
    ...
    m_FilterWidget->SetColumnWidth("风控编号", 70);
...
void RiskJudgeWidget::InitControlPannel()
{
    ...
    {
        QGroupBox* limitGroup = new QGroupBox("风控限额");
        ...
        fromLayout->addRow("风控编号：", riskIDCombo);
        ...
        fromLayout->addRow("流量限制：", flowLimitEdit);
        QLineEdit* tickerCancelLimitEdit = new QLineEdit;
        fromLayout->addRow("单证券撤单限制：", tickerCancelLimitEdit);
        QLineEdit* orderCancellLimitEdit = new QLineEdit;
        fromLayout->addRow("总撤单限制：", orderCancellLimitEdit);
    ...
    {
        QGroupBox* accountLockedGroup = new QGroupBox("账户锁定");
        QVBoxLayout* vLayout = new QVBoxLayout;
        vLayout->setContentsMargins(0, 0, 0, 0);
        QFormLayout* fromLayout = new QFormLayout;
        fromLayout->setContentsMargins(0, 0, 0, 0);
        ...
        fromLayout->addRow("账户：", accountCombo);

        QLineEdit* tickerEdit = new QLineEdit;
        fromLayout->addRow("证券代码：", tickerEdit);

        QComboBox* sideCombo = new QComboBox();
        sideCombo->addItem("全部锁定", Message::ERiskLockedSide::ELOCK_ACCOUNT);
        sideCombo->addItem("解除锁定", Message::ERiskLockedSide::EUNLOCK);
        sideCombo->addItem("禁止买入", Message::ERiskLockedSide::ELOCK_BUY);
        sideCombo->addItem("禁止卖出", Message::ERiskLockedSide::ELOCK_SELL);
        fromLayout->addRow("锁定方向：", sideCombo);
        vLayout->addLayout(fromLayout);
                ...
                [ = ]()
        {
            ...
            int LockSide = sideCombo->currentData().toInt();
...
{
    QStringList headers;
    headers << "风控编号" << "账户" << "证券代码" << "撤单次数" << "上限" << "操作员" << "更新时间";
...
{
    QStringList headers;
    headers << "风控编号" << "账户" << "证券代码" << "锁定方向" << "操作员" << "更新时间";
...
{
    QStringList headers;
    headers << "风控编号" << "账户" << "证券代码" << "操作员" << "更新时间" << "事件";
