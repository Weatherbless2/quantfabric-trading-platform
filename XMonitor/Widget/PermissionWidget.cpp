...
void SelectProtocolItem(QComboBox* comboBox, const QString& value)
{
...
PermissionWidget::PermissionWidget(QWidget *parent): FinTechUI::TabPageWidget(parent)
{
    ...
    QStringList headers;
    headers << "用户名" << "密码" << "角色" << "更新时间" << "功能权限" << "数据权限";
...

    {
        ...
        {
            QGroupBox* UserGroup = new QGroupBox("用户管理");
            ...
            fromLayout->addRow("用户名：", UserNameEdit);

            QLineEdit* PassWDEdit = new QLineEdit;
            fromLayout->addRow("密码：", PassWDEdit);

            QComboBox* OperationCombo = new QComboBox;
            AddProtocolItems(OperationCombo, QStringList() << "Add" << "Update" << "Delete");
            fromLayout->addRow("操作：", OperationCombo);

            QComboBox* RoleCombo = new QComboBox;
            AddProtocolItems(RoleCombo, QStringList() << "Monitor" << "Trader" << "Market" << "Risk" << "Admin");
            fromLayout->addRow("角色：", RoleCombo);
            ...
            connect(m_PermissionTableView, &QTableView::clicked, [ = ](const QModelIndex & index)
            {
                ...
                QString Role = UiText::Protocol(m_PermissionProxyModel->index(row, 2).data().toString());

                UserNameEdit->setText(UserName);
                PassWDEdit->setText(PassWord);
                SelectProtocolItem(RoleCombo, Role);
            });
                    ...
                    [ = ]()
            {
                ...
                QString Operation = OperationCombo->currentData().toString();
...
void PermissionWidget::UpdateUserPermissionTable(const Message::PackMessage& msg)
{
    ...
    if(it == m_PermissionMap.end())
    {
        if(Message::EPermissionOperation::EUSER_DELETE != msg.LoginResponse.Operation)
        {
            ...
            FinTechUI::XTableModelItem* RoleItem = new FinTechUI::XTableModelItem(UiText::Display(msg.LoginResponse.Role));
            ...
            FinTechUI::XTableModelItem* PluginsItem = new FinTechUI::XTableModelItem(UiText::DisplayList(msg.LoginResponse.Plugins), Qt::AlignLeft | Qt::AlignVCenter);
            ModelRow->push_back(PluginsItem);
            FinTechUI::XTableModelItem* MessagesItem = new FinTechUI::XTableModelItem(UiText::DisplayList(msg.LoginResponse.Messages), Qt::AlignLeft | Qt::AlignVCenter);
    ...
    else
    {
        ...
        else if(Message::EPermissionOperation::EUSER_UPDATE == msg.LoginResponse.Operation)
        {
            ...
            (*ModelRow)[2]->setText(UiText::Display(msg.LoginResponse.Role));
            (*ModelRow)[3]->setText(msg.LoginResponse.UpdateTime);
            (*ModelRow)[4]->setText(UiText::DisplayList(msg.LoginResponse.Plugins));
            (*ModelRow)[5]->setText(UiText::DisplayList(msg.LoginResponse.Messages));
...
void PermissionWidget::OnUpdateUserPermission(const QModelIndex &index)
{
    ...
    QString PassWord = m_PermissionProxyModel->index(row, 1).data().toString();
    QString Role = UiText::Protocol(m_PermissionProxyModel->index(row, 2).data().toString());
    QString Plugin = UiText::ProtocolList(m_PermissionProxyModel->index(row, 4).data().toString());
    QString Message = UiText::ProtocolList(m_PermissionProxyModel->index(row, 5).data().toString());
    ...
    if(index.column() == 4)
    {
        ...
        QComboBox* RoleCombo = new QComboBox;
        AddProtocolItems(RoleCombo, QStringList() << "Trader" << "Monitor" << "Admin" << "Market");
        SelectProtocolItem(RoleCombo, Role);
        ...
        for (int i = 0; i < AllPlugins.size(); i++)
        {
            ...
            checkBox->setText(UiText::Display(AllPlugins.at(i)));
            checkBox->setProperty("protocolValue", AllPlugins.at(i));
        ...
        formLayout->addRow("功能权限：", vLayout);

        QTextEdit* MessageTextEdit = new QTextEdit();
        MessageTextEdit->setText(UiText::DisplayList(Message));
        MessageTextEdit->setReadOnly(true);
        MessageTextEdit->setLineWrapMode(QTextEdit::WidgetWidth);
        formLayout->addRow("数据权限：", MessageTextEdit);
        ...
        if(dialog.exec() == QDialog::Accepted)
        {
            ...
            Role = RoleCombo->currentData().toString();
            Plugin.clear();
            Plugins.clear();
            for (int i = 0; i < PluginsVec.size(); i++)
            {
                ...
                {
                    Plugins.append(PluginsVec[i]->property("protocolValue").toString());
                }
            }
            Plugin = Plugins.join("|");
    ...
    else if(index.column() == 5)
    {
        ...
        PluginTextEdit->setText(UiText::DisplayList(Plugin));
        PluginTextEdit->setReadOnly(true);
        PluginTextEdit->setLineWrapMode(QTextEdit::WidgetWidth);
        formLayout->addRow("功能权限：", PluginTextEdit);
        ...
        {
            QCheckBox* checkBox = new QCheckBox;
            checkBox->setText(UiText::Display(AllMessages.at(i)));
            checkBox->setProperty("protocolValue", AllMessages.at(i));
        ...
        formLayout->addRow("数据权限：", vLayout);
        ...
        if(dialog.exec() == QDialog::Accepted)
        {
            ...
            for (int i = 0; i < MessagesVec.size(); i++)
            {
                ...
                {
                    Messages.append(MessagesVec[i]->property("protocolValue").toString());
                }
            }
