...
namespace
{
QWidget* CreateTableSection(const QString& title, QAbstractItemView* table)
{
    ...
    titleLabel->setContentsMargins(6, 2, 6, 2);
    titleLayout->addWidget(titleLabel);
    titleLayout->addStretch();

    QLabel* countLabel = new QLabel(section);
    countLabel->setContentsMargins(6, 2, 6, 2);
    titleLayout->addWidget(countLabel);
    layout->addLayout(titleLayout);
    layout->addWidget(table);

    QAbstractItemModel* model = table->model();
    auto updateCount = [model, countLabel]()
    {
        countLabel->setText(QString("记录: %1").arg(model->rowCount()));
    };
    QObject::connect(model, &QAbstractItemModel::rowsInserted, section, updateCount);
    QObject::connect(model, &QAbstractItemModel::rowsRemoved, section, updateCount);
    QObject::connect(model, &QAbstractItemModel::modelReset, section, updateCount);
    updateCount();
    return section;
}

void AddProtocolItems(QComboBox* comboBox, const QStringList& values)
{
    for(const QString& value : values)
    {
        comboBox->addItem(UiText::Display(value), value);
    }
}
}
...
{   
    ...
    // Keep each data region identifiable even when the corresponding model is empty.
    m_MiddleSplitter->addWidget(CreateTableSection("股票持仓", m_StockTickerPositionTableView));
    m_MiddleSplitter->addWidget(CreateTableSection("期货持仓", m_FutureTickerPositionTableView));
    m_MiddleSplitter->addWidget(CreateTableSection("当前挂单", m_HangingOrderTableView));
    m_MiddleSplitter->addWidget(CreateTableSection("历史委托", m_HistoryOrderTableView));
...
void OrderManagerWidget::OnSendOrder()
{
    ...
    formLayout->addRow("产品：", productCombo);
    ...
    formLayout->addRow("账户：", accountCombo);
    ...
    formLayout->addRow("证券代码：", TickerCombo);
    ...
    formLayout->addRow("交易所：", ExchangeCombo);

    QComboBox* orderTypeCombo = new QComboBox;
    orderTypeCombo->addItem("立即成交剩余撤销（FAK）", "FAK");
    orderTypeCombo->addItem("立即全部成交否则撤销（FOK）", "FOK");
    orderTypeCombo->addItem("限价", "LIMIT");
    formLayout->addRow("委托类型：", orderTypeCombo);

    QComboBox* directionCombo = new QComboBox;
    AddProtocolItems(directionCombo, QStringList() << "Buy" << "Sell" << "ReverseRepo" << "Subscription" << "Allotment"
                                                   << "CollateralTransferIn" << "CollateralTransferOut" << "MarginBuy" << "RepayMarginBySell"
                                                   << "ShortSell" << "RepayStockByBuy" << "RepayStockDirect");
    formLayout->addRow("买卖方向：", directionCombo);
    QComboBox* offsetCombo = new QComboBox;
    AddProtocolItems(offsetCombo, QStringList() << "None" << "Open" << "Close" << "CloseToday" << "CloseYestoday");
    formLayout->addRow("开平标志：", offsetCombo);
    QComboBox* riskCheckCombo = new QComboBox;
    AddProtocolItems(riskCheckCombo, QStringList() << "Check" << "NoCheck");
    formLayout->addRow("风控检查：", riskCheckCombo);
    QComboBox* engineCombo = new QComboBox;
    AddProtocolItems(engineCombo, QStringList() << "None" << "TraderOrder");
    formLayout->addRow("委托来源：", engineCombo);
    QLineEdit* priceEdit = new QLineEdit;
    formLayout->addRow("价格：", priceEdit);
    QLineEdit* volumeEdit = new QLineEdit;
    formLayout->addRow("数量：", volumeEdit);
    QLineEdit* countEdit = new QLineEdit;
    countEdit->setText("1");
    formLayout->addRow("发送次数：", countEdit);
...
void OrderManagerWidget::OnTransferFund()
{
    ...
    QComboBox* directionCombo = new QComboBox;
    AddProtocolItems(directionCombo, QStringList() << "In" << "Out");
    formLayout->addRow("方向：", directionCombo);
...
void OrderManagerWidget::InitFilterWidget()
{
    ...
    m_FilterWidget->SetHeaderLabels(QStringList() << "机房" << "产品" << "账户" << "证券代码");
    m_FilterWidget->SetColumnWidth("机房", 60);
    m_FilterWidget->SetColumnWidth("产品", 70);
...
void OrderManagerWidget::InitControlPannel()
{
    ...
    QPushButton* sendOrderButton = new QPushButton("委托下单");
    QPushButton* cancelOrderButton = new QPushButton("撤销委托");
    QPushButton* exportOrderTableButton = new QPushButton("导出委托");
    QPushButton* transferFundButton = new QPushButton("资金划转");
    QPushButton* repayMarginButton = new QPushButton("直接还款");
...
void OrderManagerWidget::InitStockTickerPositionTable()
{
    ...
    headerData << "机房" << "柜台" << "产品" << "账户" << "证券代码" << "交易所" << "昨日多仓" << "多仓总量"
               << "今日买入" << "今日卖出" << "昨日融资仓" << "融资仓总量" << "今日融资买入" << "今日卖券还款"
               << "昨日融券仓" << "融券仓总量" << "今日融券卖出" << "今日买券还券" << "直接还券" << "专项头寸可用" << "更新时间";
...
{
    QStringList headerData;
    headerData << "机房" << "柜台" << "产品" << "账户" << "合约代码" << "交易所" << "今日多仓" << "昨日多仓"
               << "开多数量" << "开多冻结" << "平今多冻结" << "平昨多冻结" << "今日空仓"
               << "昨日空仓" << "开空数量" << "开空冻结" << "平今空冻结" << "平昨空冻结" << "更新时间";
...
{
    QStringList headerData;
    headerData << "机房" << "柜台" << "产品" << "账户" << "证券代码" << "交易所" << "数量" << "价格" << "状态"
               << "买卖方向" << "委托类型" << "引擎编号" << "委托引用" << "柜台委托号" << "本地委托号" << "委托令牌"
               << "风控编号" << "错误码" << "错误信息" << "接收行情时间" << "发送时间" << "报单时间" << "柜台确认时间" << "交易所确认时间" << "更新时间";
...
void OrderManagerWidget::InitAccountFundTable()
{
    ...
    QStringList headerData;
    headerData << "账户" << "可用资金" << "总资产" << "入金" << "出金" << "手续费" << "可取资金";
...
void OrderManagerWidget::CancelOrderDialog(const QString& account, const QString& OrderID)
{
    ...
    formLayout->addRow("委托引用：", OrderRefEdit);
    ...
    QComboBox* engineCombo = new QComboBox;
    AddProtocolItems(engineCombo, QStringList() << "None" << "TraderOrder" << "ForceEndOrder");
    formLayout->addRow("委托来源：", engineCombo);
    ...
    if(dialog.exec() == QDialog::Accepted)
    {
        ...
        else if(engineCombo->currentData().toString() == "ForceEndOrder")
        {
