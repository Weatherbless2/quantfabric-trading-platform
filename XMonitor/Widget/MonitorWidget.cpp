...
void MonitorWidget::InitFilterWidget()
{
    ...
    m_FilterWidget->SetColumnWidth("应用", 140);
}
...
void MonitorWidget::InitAppStatusTableView()
{
    ...
    headerData << "机房" << "应用名称" << "账户" << "进程号" << "状态" << "停止" << "启动" << "CPU使用率" << "内存占用"
               << "启动时间" << "上次启动" << "提交版本" << "工具库版本" << "接口版本" << "更新时间" << "启动脚本";
    ...
    FinTechUI::XButtonDelegate* StopButtonDelegate = new FinTechUI::XButtonDelegate("停止");
    FinTechUI::XButtonDelegate* StartButtonDelegate = new FinTechUI::XButtonDelegate("启动");
