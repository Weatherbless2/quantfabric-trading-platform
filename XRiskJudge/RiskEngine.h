...
protected:
    ...
    // 资金、可卖持仓和已通过未终结委托的冻结检查
    bool AccountCapacity(Message::PackMessage& msg);
    void UpdateAccountFund(const Message::TAccountFund& fund);
    void UpdateAccountPosition(const Message::TAccountPosition& position);
    void ReleaseOrderReservation(const Message::TOrderStatus& status);
    static std::string OrderTokenKey(const char* account, uint64_t orderToken);
    // 打印风控检查前后报单请求的状态
...
private:
    ...
    std::unordered_map<std::string, long long> m_AccountFlowWindowStartMap;// Account, rolling window start
    std::unordered_map<std::string, Message::TAccountFund> m_AccountFundMap;// Account, latest counter snapshot
    std::unordered_map<std::string, Message::TAccountPosition> m_AccountPositionMap;// Account:Ticker, latest snapshot
    std::unordered_map<std::string, double> m_ReservedBuyAmountMap;// Account, passed buy notional
    std::unordered_map<std::string, int> m_ReservedSellVolumeMap;// Account:Ticker, passed sell volume
    std::unordered_map<std::string, std::pair<double, int>> m_OrderReservationMap;// Account:OrderToken, buy amount/sell volume
