...
void HPPackClient::Start()
{
    ...
    else
    {
        FMTLOG(fmtlog::WRN, "HPPackClient::Start connected to server[{}:{}] failed, error code:{} error massage:{}",
                m_ServerIP, m_ServerPort, static_cast<int>(::HP_Client_GetLastError(m_pClient)),
                HP_Client_GetLastErrorDesc(m_pClient));
    }
}

void HPPackClient::Login(const Message::TLoginRequest& login)
{
...
void HPPackClient::SendData(const unsigned char* pBuffer, int iLength)
{
    ...
    if (!ret)
    {
        FMTLOG(fmtlog::WRN, "HPPackClient::SendData failed, sys error code:{}, error code:{}, error message:{}",
                SYS_GetLastError(), static_cast<int>(HP_Client_GetLastError(m_pClient)),
                HP_Client_GetLastErrorDesc(m_pClient));
    }
}
...
En_HP_HandleResult __stdcall HPPackClient::OnConnect(HP_Client pSender, HP_CONNID dwConnID)
{
    ...
    m_Connected = true;
    FMTLOG(fmtlog::INF, "HPPackClient::OnConnect {}:{} connected to server, dwConnID:{}", szAddress, usPort, dwConnID);
    return HR_OK;
}
